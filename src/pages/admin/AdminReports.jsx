...
import {
  ...
  Radio, Calendar, Star, User, Hash, Loader2,
} from 'lucide-react';
    ...
    <div className="flex h-[calc(100vh-3.5rem)] md:h-[calc(100vh-4rem)] overflow-hidden">
      ...
      {!selected ? (
        <div className="hidden md:flex flex-col w-1/2 items-center justify-center bg-background">
          ...
          <p className="text-sm text-muted-foreground font-medium">Sélectionnez un signalement</p>
          <p className="text-xs text-muted-foreground/60 mt-0.5">Choisissez un signalement dans la liste pour l'examiner.</p>
        </div>
      ) : (
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden min-w-0 p-3 md:p-4 space-y-3">
              ...
              <div className="rounded-xl bg-secondary/30 border border-border p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="font-mono text-[10px] truncate overflow-hidden">{selected.target_id || '—'}</p>
                  </div>
                  <div className="min-w-0">
                ...
                {selected.target_context && (
                  <div className="border-t border-border pt-2 min-w-0">
              ...
              {selected.target_url && (
                <a href={selected.target_url} target="_blank" rel="noreferrer"
                  ...
                  <span className="flex-shrink-0">Voir le contenu signalé</span>
                  <span className="text-muted-foreground ml-auto text-[10px] truncate min-w-0">{selected.target_url}</span>
                </a>
              )}

              {/* Reason + details */}
              <div className="rounded-xl bg-secondary/30 border border-border p-3 space-y-2 min-w-0">
                <div className="flex items-center gap-2">
                ...
                {selected.details && (
                  <div className="border-t border-border pt-2 min-w-0">
                    ...
                    <p className="text-xs text-foreground/80 break-words">{selected.details}</p>
                  </div>
                )}
              </div>
              ...
              {(selected.message_content || selected.target_content_preview) && (
                <div className="rounded-xl bg-destructive/5 border border-destructive/20 p-3 min-w-0">
                  <p className="text-[10px] font-mono text-muted-foreground/60 uppercase tracking-wider mb-1.5">Contenu signalé (snapshot)</p>
                  <p className="text-xs text-foreground/90 italic whitespace-pre-wrap break-words">"{selected.message_content || selected.target_content_preview}"</p>
                </div>
              )}
