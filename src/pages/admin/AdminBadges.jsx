           ...
           <div key={u.id} className="bg-card border border-border rounded-xl overflow-hidden hover:border-primary/20 transition-colors">
             ...
             {expandedUserId === u.id && (
               <div className="border-t border-border bg-secondary/40 p-3 sm:p-4 space-y-4">
                 {/* Verifications grid */}
                 <div>
                   <p className="font-inter text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Certifications</p>
                     ...
                     {VERIFICATION_TYPES.map(vt => {
                       ...
                       return (
                         <button
                           ...
                           title={vt.desc}
                           className={`flex items-center justify-center gap-1 rounded-lg px-2.5 py-2 border transition-all text-center ${active ? `${vt.bg} ${vt.border}` : 'bg-background border-border hover:border-primary/30'}`}
                         >
                           <Icon className={`w-3.5 h-3.5 flex-shrink-0 ${active ? vt.color : 'text-muted-foreground'}`} />
                           <span className={`font-inter text-[10px] sm:text-xs font-semibold ${active ? vt.color : 'text-muted-foreground'}`}>{vt.label}</span>
                         </button>
                       );
                     })}
                   </div>
                 ...
                 {/* Badges grid */}
                 <div>
                   <p className="font-inter text-xs font-semibold text-muted-foreground mb-2 uppercase tracking-wide">Badges</p>
                   <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                     {ALL_BADGES.map(badge => {
                       ...
                       return (
                         <button
                           ...
                           className={`flex items-center justify-center gap-1 font-inter text-[10px] sm:text-xs px-2.5 py-2 rounded-lg border transition-all ${
                             hasBadge
