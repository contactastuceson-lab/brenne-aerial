...
import AdminRegisterDialog from '@/components/admin/events/AdminRegisterDialog';
...
export default function AdminEvents() {
  ...
  const confirmCancelReg = async (reg) => {
    if (!confirm(`Désinscrire ${reg.user_name || reg.user_email} de « ${reg.event_title} » ? (sans remboursement)`)) return;
    setBusy(true);
    try {
      await base44.functions.invoke('adminManageEvent', { action: 'cancel_registration', registration_id: reg.id });
      toast.success('Inscription annulée (sans remboursement)');
      loadAll();
    } catch { toast.error('Échec de la désinscription'); }
    setBusy(false);
  };
  ...
  return (
    <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-2.5">
                          <div className="w-10 h-10 rounded-lg overflow-hidden bg-secondary flex-shrink-0">
                            {ev.image_url
                              ...
                              : <div className="w-full h-full flex items-center justify-center"><Calendar className="w-4 h-4 text-muted-foreground/40" /></div>}
                          </div>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <div className="flex items-center gap-3 ml-auto">
              ...
              <Button size="sm" onClick={() => setRegisterOpen(true)}>
                <Plus className="w-4 h-4" /> Inscrire un utilisateur
              </Button>
            </div>
          </div>
                    ...
                    <tr key={reg.id} className="hover:bg-secondary/20">
                      ...
                      <td className="p-3 text-right">
                        {reg.status === 'registered' && (
                          <div className="flex items-center gap-1 justify-end">
                            <button onClick={() => confirmCancelReg(reg)} title="Désinscrire (sans remboursement)"
                              className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-muted/40 text-muted-foreground hover:bg-muted/60 text-xs font-grotesk font-bold">
                              <Ban className="w-3.5 h-3.5" /> Désinscrire
                            </button>
      ...
      {/* Register dialog */}
      <AdminRegisterDialog open={registerOpen} onClose={() => setRegisterOpen(false)} events={events} onDone={loadAll} />
