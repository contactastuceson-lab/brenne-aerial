...
function emailTemplate({ title, preheader, bodyHtml, ctaUrl, ctaLabel }) {
  return `<!DOCTYPE html>
<html lang="fr">
...
Deno.serve(async (req) => {
  ...
  for (const admin of admins) {
    if (admin.email) {
      await base44.integrations.Core.SendEmail({
        to: admin.email,
        subject: `🗑️ Nouvelle demande de suppression — ${user.full_name || user.email}`,
        body: emailTemplate({
          title: 'Demande de suppression de compte',
          preheader: `${user.full_name || user.email} souhaite supprimer son compte.`,
          bodyHtml: `
            <p style="color:#8aaec8;font-size:15px;line-height:1.7;margin:0 0 20px;">Un utilisateur a demandé la suppression de son compte.</p>
