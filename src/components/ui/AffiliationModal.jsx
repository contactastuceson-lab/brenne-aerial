...
export default function AffiliationModal({ user, open, onOpenChange }) {
  ...
  const certifiedDate = formatCertifiedDate(affiliation);
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" hideClose className="fixed inset-x-0 bottom-0 w-full max-w-none rounded-t-3xl border-t border-border bg-card p-0 shadow-2xl">
        <div className="mx-auto w-full max-w-2xl px-5 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] pt-5">
          ...
          <p className="mb-5 text-left text-[10px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Affiliation officielle</p>
          <div className="flex flex-col gap-3 text-left">
            <div className="flex items-start gap-3">
              <VerificationMark type="certified" size="2em" marginLeft="0" className="mt-0.5 flex-shrink-0" />
              <p className="text-sm leading-6 text-foreground">Ce compte est <strong>Certifié</strong>, car c'est un affilié de <strong>@{organizationHandle}</strong> sur la plateforme.</p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm leading-6 text-foreground"><strong>{formatRole(affiliation.role)}</strong>{certifiedDate ? ` · Compte certifié depuis ${certifiedDate}` : ''}</p>
            </div>
          </div>
        </div>
      </SheetContent>
