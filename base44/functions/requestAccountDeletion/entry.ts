...
function supremeEmailTemplate(userName) {
  return `<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Brenne Aerial — Rang Suprême</title></head>
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td align="center" style="background:linear-gradient(135deg,#1a0c00,#2d1500,#1a0c00);border-radius:16px 16px 0 0;padding:40px 40px 28px;border-bottom:1px solid rgba(217,119,6,0.4);">
          ...
          <p style="margin:0;font-size:10px;letter-spacing:3px;color:rgba(217,119,6,0.6);font-weight:600;text-transform:uppercase;">Brenne Aerial — Services Élite</p>
        </td></tr>
        ...
        <tr><td style="background:linear-gradient(145deg,#1a0c00,#150a00);padding:48px 40px;">
          ...
          <p style="color:#a08040;font-size:14px;line-height:1.8;margin:0 0 24px;">En tant que membre <strong style="color:#f59e0b;">Suprême</strong> de la communauté Brenne Aerial, votre demande de suppression de compte a été reçue avec toute l'attention et le respect qu'elle mérite.</p>
          ...
          <p style="color:#806030;font-size:13px;line-height:1.8;margin:0;">Si cette demande est une erreur, contactez immédiatement notre équipe à <a href="mailto:contact@brenneaerial.fr" style="color:#d97706;text-decoration:none;font-weight:600;">contact@brenneaerial.fr</a>. Nous ferons notre possible pour annuler le processus.</p>
        </td></tr>
        ...
        <tr><td align="center" style="background:linear-gradient(135deg,#1a0c00,#0d0800);border-radius:0 0 16px 16px;padding:28px 40px;">
          <p style="margin:0 0 4px;font-size:10px;letter-spacing:3px;color:rgba(217,119,6,0.6);text-transform:uppercase;">Brenne Aerial · Rang Suprême</p>
          <p style="margin:0;font-size:11px;color:rgba(139,90,30,0.7);">Brenne, Creuse, France · <a href="mailto:contact@brenneaerial.fr" style="color:#d97706;text-decoration:none;">contact@brenneaerial.fr</a></p>
        </td></tr>
...
Deno.serve(async (req) => {
  ...
  if (isSupreme) {
    await base44.integrations.Core.SendEmail({
      to: user.email,
      subject: '👑 Rang Suprême — Votre demande a été transmise · Brenne Aerial',
      body: supremeEmailTemplate(user.full_name || 'cher membre'),
    });
  } else {
    await base44.integrations.Core.SendEmail({
      to: user.email,
      subject: '⚠️ Demande de suppression de compte reçue — Brenne Aerial',
      body: standardEmailTemplate({
        ...
        bodyHtml: `
          ...
          <p style="color:#8aaec8;font-size:15px;line-height:1.7;margin:0 0 16px;">Nous avons bien reçu votre demande de <strong style="color:#e8f4fc;">suppression définitive</strong> de votre compte Brenne Aerial.</p>
          ...
          <p style="color:#8aaec8;font-size:14px;line-height:1.7;margin:0;">Si vous avez fait cette demande par erreur, contactez-nous rapidement à <a href="mailto:contact@brenneaerial.fr" style="color:#3ab0dc;">contact@brenneaerial.fr</a></p>
        `,
      }),
    });
  }
    ...
    if (admin.email) {
      await base44.integrations.Core.SendEmail({
        ...
        body: standardEmailTemplate({
          ...
          `,
          ctaUrl: 'https://app.brenneaerial.fr/admin/users',
