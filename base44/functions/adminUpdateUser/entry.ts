...
Deno.serve(async (req) => {
  try {
    ...
    // Send email if account status changed to banned/suspended/restricted
    const statusChanged = data.account_status && ['banned', 'suspended', 'restricted'].includes(data.account_status);
    const wasAlreadySameStatus = targetUser?.account_status === data.account_status;
    if (statusChanged && !wasAlreadySameStatus && targetUser?.email) {
      try {
        const isSupreme = (targetUser.verifications || []).includes('supreme');
        const html = isSupreme
