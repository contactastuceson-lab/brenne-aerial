...
import { Wrench, Plane, Radio, Zap, Clock } from 'lucide-react';
...
const CHECKS = [
  { label: 'Systèmes de navigation', done: true },
  { label: 'Modules de captation 4K', done: true },
  { label: 'Interface client', done: false, active: true },
  { label: 'Déploiement final', done: false },
];
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center relative overflow-hidden px-6">
      ...
      >
        ...
        <p className="font-inter text-muted-foreground text-sm leading-relaxed mt-4 mb-8 max-w-sm">
          {message || "Nous effectuons des améliorations pour vous offrir la meilleure expérience possible. Retour imminent."}
        </p>
            ...
            {CHECKS.map((check, i) => (
              ...
              >
                ...
                <span className={`font-inter text-xs ${
                  check.done ? 'text-muted-foreground line-through' : check.active ? 'text-foreground font-medium' : 'text-muted-foreground/50'
                }`}>
                  {check.label}
                </span>
                {check.active && (
                ...
                {check.done && (
                  <span className="ml-auto font-mono text-[9px] text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full">OK</span>
                )}
              </motion.div>
            ))}
          </div>
        ...
        <div className="w-full mb-6">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="font-mono text-[10px] text-primary">68%</span>
          </div>
